...
# 💡 HIERARCHICAL CLASSIFICATION CONSTANTS - Config에서 동적으로 로드
# 전역 변수들은 initialize_hierarchical_constants() 함수에서 초기화됨
HARD_CLASSES = None
...
def initialize_hierarchical_constants(cfg_a):
    ...
    # Model A 맵핑 생성
    total_classes = getattr(cfg_a, 'num_classes', 17)
    ...
    MODEL_A_CLASS_MAP = model_a_map
    INV_MODEL_A_CLASS_MAP = {v: k for k, v in model_a_map.items()}
    
    print(f"🔧 계층적 분류 상수 초기화 완료:")
    print(f"   HARD_CLASSES: {HARD_CLASSES}")
    print(f"   Total Classes: {total_classes}")
    
    return HARD_CLASSES, MODEL_B_CLASS_MAP, INV_MODEL_B_CLASS_MAP, MODEL_A_CLASS_MAP, INV_MODEL_A_CLASS_MAP
...
def train_hierarchical_model(model_type, cfg, df, run, train_transforms, val_transform, val_tta_transform):
    ...
    if cfg.n_folds >= 3:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_df_filtered, train_df_filtered['target'])):
            ...
            try:
                ...
                _, val_f1 = do_validation(
                    ...
                    savepath=os.path.join(cfg.submission_dir, f"val_confusion_matrix{'_TTA' if cfg.val_TTA else ''}_Model-{model_type}_Fold{fold}.png"),
        ...
        try:
            final_trainer, final_aug_ids, final_val_aug_ids, _, _ = run_training_cycle(
        ...
        try:
            ...
            train_split_df, val_split_df = train_test_split(
                train_df_filtered, test_size=cfg.val_split_ratio, random_state=cfg.random_seed, stratify=train_df_filtered['target']
            )
...
if __name__ == "__main__":
    try:
        ...
        # 🔧 계층적 분류 상수들 초기화 (Config 기반)
        initialize_hierarchical_constants(cfg_a)
        ...
        next_run_name = (
            ...
            f"{cfg_a.model_name.split('.')[0]}-"  # 모델명 간소화
            f"opt{cfg_a.optimizer_name}-"
            f"sch{cfg_a.scheduler_name}-"
            f"img{cfg_a.image_size}-"
            f"es{cfg_a.patience}-"
            f"{aug_str_parts}-"
            f"cv{cfg_a.n_folds}-"
            f"bs{cfg_a.batch_size}-"
            f"vTTA{1 if cfg_a.val_TTA else 0}-"
            f"tTTA{1 if cfg_a.test_TTA else 0}-"
            f"MP{1 if cfg_a.mixed_precision else 0}"
        )
        # Model B run name 생성
        next_run_name2 = (
            f"{CURRENT_TIME}-ModelB-"
            f"{cfg_b.model_name.split('.')[0]}-"  # 모델명 간소화
            f"opt{cfg_b.optimizer_name}-"
            f"sch{cfg_b.scheduler_name}-"
            f"img{cfg_b.image_size}-"
            f"es{cfg_b.patience}-"
            f"{'mixup' if cfg_b.online_aug['mixup'] else 'cutmix'}-"  
            f"cv{cfg_b.n_folds}-"
            f"bs{cfg_b.batch_size}-"
            f"vTTA{1 if cfg_b.val_TTA else 0}-"
            f"tTTA{1 if cfg_b.test_TTA else 0}-"
            f"MP{1 if cfg_b.mixed_precision else 0}"
        )

        # Model A WanDB 설정 - model_name 기반 프로젝트
        run = None 
        if hasattr(cfg_a, 'wandb') and cfg_a.wandb['log']:
            # model_name 기반 프로젝트명 생성
            if getattr(cfg_a.wandb, 'model_based_project', True):
                model_a_name_clean = cfg_a.model_name.replace('.', '-').replace('_', '-')
                project_name_a = f"{cfg_a.wandb['project']}-modelA-{model_a_name_clean}"
            else:
                project_name_a = cfg_a.wandb['project']
            
            # 태그 설정
            tags_a = getattr(cfg_a.wandb, 'tags', [])
            tags_a.extend(["model_a", cfg_a.model_name, cfg_a.optimizer_name, cfg_a.scheduler_name])
            
            run = wandb.init(
                project=project_name_a,
                name=next_run_name,
                config=vars(cfg_a),
                tags=tags_a
            )
        ...
        # Model B WanDB 설정 - model_name 기반 프로젝트
        run = None 
        if hasattr(cfg_b, 'wandb') and cfg_b.wandb['log']:
            # model_name 기반 프로젝트명 생성
            if getattr(cfg_b.wandb, 'model_based_project', True):
                model_b_name_clean = cfg_b.model_name.replace('.', '-').replace('_', '-')
                project_name_b = f"{cfg_b.wandb['project']}-modelB-{model_b_name_clean}"
            else:
                project_name_b = cfg_b.wandb['project']
            
            # 태그 설정
            tags_b = getattr(cfg_b.wandb, 'tags', [])
            tags_b.extend(["model_b", cfg_b.model_name, cfg_b.optimizer_name, cfg_b.scheduler_name])
            
            run = wandb.init(
                project=project_name_b,
                name=next_run_name2,
                config=vars(cfg_b),
                tags=tags_b
            )
