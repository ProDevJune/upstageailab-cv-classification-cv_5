...
def get_criterion(cfg, class_weights=None):
    CRITERIONS = {
        ...
        "LabelSmoothingLoss": LabelSmoothingLoss(classes=cfg.num_classes, smoothing=cfg.label_smooth, weight=class_weights)
    }
...
def plot_cross_validation(
    ...
    for i in range(cfg.n_folds):
        ...
        plt.plot(range(1, len(train_losses)+1), train_losses, label=f"Fold-{i+1} Train {title}")
        plt.plot(range(1, len(val_losses)+1), val_losses, label=f"Fold-{i+1} Val {title}")
