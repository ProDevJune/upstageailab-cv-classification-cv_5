...
AUG = {
    ...
    'aggressive': A.Compose([
        ...
        A.CoarseDropout(
            ...
            max_height=35,
            max_width=45,
            fill_value=0,  # fill 대신 fill_value 사용
            p=0.9
        ),
        # 강력한 기하학적 변환
        A.OneOf([
            ...
            A.Perspective(scale=(0.05, 0.1), p=1.0),  # value 제거
        ], p=0.9),
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.Transpose(p=0.5),
...
def get_augmentation(cfg, epoch=0):
    common_resize_transform = A.Compose([
        ...
        A.PadIfNeeded(min_height=cfg.image_size, min_width=cfg.image_size, border_mode=cv2.BORDER_CONSTANT, p=1.0),  # value 제거
