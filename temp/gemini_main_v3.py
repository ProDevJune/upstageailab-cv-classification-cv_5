...
#📢 project_root 설정 - cv-classification 환경에 맞게 수정
...
if __name__ == "__main__":
    try:
        ...
        parser = argparse.ArgumentParser(description="🔥 Phase 1: Run advanced deep learning training with Focal Loss, Label Smoothing, CutMix/MixUp")
        parser.add_argument(
            '--config',
            type=str,
            default='config_v3.yaml', # 🔥 Phase 1: v3 기본 설정
            help='Name of the configuration YAML file (e.g., config_v3.yaml, experiment_v3.yaml)'
        )
        ...
        # 🔥 Phase 1: 고급 기법 정보 출력
        print(f"🔥 Phase 1 고급 기법 적용:")
        print(f"   - 손실함수: {cfg.criterion}")
        if cfg.criterion == 'FocalLoss':
            focal_config = getattr(cfg, 'focal_loss', {})
            print(f"     * alpha: {focal_config.get('alpha', 1.0)}, gamma: {focal_config.get('gamma', 2.0)}")
        elif cfg.criterion == 'LabelSmoothingCrossEntropy':
            label_smooth_config = getattr(cfg, 'label_smoothing', {})
            print(f"     * smoothing: {label_smooth_config.get('smoothing', 0.1)}")
        
        mixup_cutmix_config = getattr(cfg, 'mixup_cutmix', {})
        mixup_cutmix_prob = mixup_cutmix_config.get('prob', 0)
        print(f"   - MixUp/CutMix: {'Enabled' if mixup_cutmix_prob > 0 else 'Disabled'}")
        if mixup_cutmix_prob > 0:
            print(f"     * 적용확률: {mixup_cutmix_prob}, switch_prob: {mixup_cutmix_config.get('switch_prob', 0.5)}")
            print(f"     * mixup_alpha: {mixup_cutmix_config.get('mixup_alpha', 1.0)}, cutmix_alpha: {mixup_cutmix_config.get('cutmix_alpha', 1.0)}")
        ...
        # 🔥 Phase 1: 고급 기법 정보를 run name에 추가
        criterion_short = cfg.criterion[:4] if cfg.criterion != 'CrossEntropyLoss' else 'CE'
        mixup_cutmix_short = f"MC{int(mixup_cutmix_prob*100)}" if mixup_cutmix_prob > 0 else "MC0"
        ...
        next_run_name = (
            ...
            f"{criterion_short}-"  # 🔥 손실함수 정보
            f"{mixup_cutmix_short}"  # 🔥 MixUp/CutMix 정보
        )
        ...
        criterion = get_criterion(cfg)  # 🔥 Phase 1: 고급 손실함수 지원
        ...
        print(f"📢 Phase 1 Validation F1-score: {val_f1:.4f}")
        ...
        print(f"🔥 Phase 1 Submission file saved to {submission_path}")
        ...
        print(f"      - 손실함수: {cfg.criterion}")
        print(f"      - MixUp/CutMix: {'Enabled' if mixup_cutmix_prob > 0 else 'Disabled'}")
        print(f"   📁 Results saved to: {cfg.submission_dir}")
